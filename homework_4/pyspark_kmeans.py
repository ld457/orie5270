...
def pyspark_kmeans(data_txt, centroid_txt):
    ...
    data = sc.textFile('data.txt').map(lambda line: np.array([float(x) for x in line.split(' ')])).cache()
    centroid = sc.textFile('c1.txt').map(lambda line: np.array([float(x) for x in line.split(' ')])).collect()
...
if __name__ == '__main__':
    pyspark_kmeans(data_txt='A.txt', centroid_txt='c1.txt')
